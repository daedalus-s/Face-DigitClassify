...
class NaiveBayesClassifier(classificationMethod.ClassificationMethod):
  ...
  def getFeatureCountFalse(self, feature, label):
    return self.clabels[label] - self.featureCounts[label][feature]
  ...
  def trainAndTune(self, trainingData, trainingLabels, validationData, validationLabels, kgrid):
    ...
    self.clabels = [0 for x in self.legalLabels]
    ...
    for i in range(len(trainingData)):
      counter += 1
      self.clabels[trainingLabels[i]] += 1
  ...
  def calculateLogJointProbabilities(self, datum):
    ...
    "*** YOUR CODE HERE ***"
    for label in self.legalLabels:
      p = math.log(self.clabels[label] / self.dataCount)

      labelprobfeature = 0
      for feature in datum:
        tC = self.getFeatureCountTrue(feature, label) + self.k
        fC = self.getFeatureCountFalse(feature, label) + self.k
        denominator = tC + fC

        if(datum[feature]):
          labelprobfeature += math.log(tC / denominator)
        else:
          labelprobfeature += math.log(fC / denominator)

      logJoint[label] = p + labelprobfeature
